<?php
    ...
    $current_page = basename($_SERVER['PHP_SELF']);
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['id'])) {
            $id = intval($_GET['id']);
            $query = "SELECT * FROM diary_entries WHERE id = ? AND user_id = ?";
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (!empty($title) && !empty($content)) {
                $query = "UPDATE diary_entries SET title = ?, content = ? WHERE id = ? AND user_id = ?";
                $stmt = $conn->prepare($query);
                $stmt->bind_param("ssii", $title, $content, $id, $_SESSION['user_id']);
                if ($stmt->execute()) {
                    echo "<script>alert('Diary entry updated successfully!'); window.location.href = 'diary.php'; </script>";
                } else {
